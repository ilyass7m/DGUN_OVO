...
class mergeblock(nn.Module):
    ...
    def forward(self, x, bridge):
        ...
        # Cast to float32 and add regularization for numerical stability (AMP compatible)
        mat_f32 = mat.float()
        # Add small identity for regularization to prevent singular matrix
        reg = 1e-4 * torch.eye(mat_f32.shape[-1], device=mat_f32.device, dtype=mat_f32.dtype)
